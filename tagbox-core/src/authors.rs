...
impl AuthorManager {
    ...
    pub async fn get_author(&self, author_id_param: &str) -> Result<Author> {
        ...
        .ok_or_else(|| TagboxError::Config(
            format!("作者ID不存在: {}", author_id_param)
        ))?;
    ...
    /// 添加作者别名
    pub async fn add_author_alias(&self, canonical_author_id: &str, alias_author_id: &str) -> Result<()> {
        ...
        let existing_alias_mapping = sqlx::query!(
            ...
            alias_author_id
        ).fetch_optional(&self.db_pool).await.map_err(TagboxError::Database)?;

        if let Some(existing_map) = existing_alias_mapping {
            // existing_map.canonical_id is String (NOT NULL in db)
            if existing_map.canonical_id == canonical_author_id { // Direct comparison for String
                return Ok(()); // Already correctly mapped
            } else {
                return Err(TagboxError::Config(format!(
                    "Author {} is already an alias for {}.", alias_author_id, existing_map.canonical_id
                )));
            }
        }
    ...
    fn compute_name_similarity(&self, name1: &str, name2: &str) -> f32 {
        ...
        // 去除空格并转为小写进行比较
        let name1_norm: String = name1.to_lowercase().chars().filter(|c| !c.is_whitespace()).collect();
        let name2_norm: String = name2.to_lowercase().chars().filter(|c| !c.is_whitespace()).collect();
        ...
        // 简单的字符匹配比例
        let common_chars = name1_norm.chars()
