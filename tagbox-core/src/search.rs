...
impl Searcher {
    ...
    pub async fn new(config: AppConfig, db_pool: SqlitePool) -> Self {
        ...
        Self { config, db_pool, fts5_signal_available }
    }
    ...
    pub async fn rebuild_fts_index(&self) -> Result<()> {
        ...
        // 获取所有文件
        let file_ids: Vec<String> = sqlx::query_scalar("SELECT id FROM files WHERE is_deleted = 0;")
    ...
    pub async fn search_advanced(
        ...
        if !parsed.exclude_tags.is_empty() {
            where_clauses.push(format!("f.id NOT IN (
        ...
        if let Some(category) = &parsed.category {
            where_clauses.push("(f.category1 = ? OR f.category2 = ? OR f.category3 = ?)".to_string());
        ...
        debug!("执行SQL查询: {}", sql);
        debug!("参数: {:?}", params.iter().map(|p| p.to_string()).collect::<Vec<String>>());
        ...
        for row in rows {
            ...
            let entry = FileEntry {
                ...
                original_path: row.get::<Option<String>, _>("original_path")
                    .map(PathBuf::from),
                ...
                updated_at: chrono::DateTime::parse_from_rfc3339(row.get::<&str, _>("updated_at"))
                    .unwrap_or_else(|_| chrono::Utc::now().into())
                    .with_timezone(&chrono::Utc),
                last_accessed: row.get::<Option<&str>, _>("last_accessed")
                    .and_then(|t| {
