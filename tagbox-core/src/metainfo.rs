...
impl MetaInfoExtractor {
    ...
    pub async fn extract(&self, file_path: &Path) -> Result<ImportMetadata> {
        ...
        // 最后，尝试从PDF元数据中提取（如果是PDF文件且配置允许）
        if self.config.import.metadata.fallback_pdf
           && file_path.extension().map_or(false, |ext| ext == "pdf") {
            if let Ok(pdf_metadata) = self.extract_from_pdf(file_path) {
                metadata = self.merge_metadata(metadata, pdf_metadata);
            }
        }
    ...
    fn extract_from_filename(&self, path: &Path) -> ImportMetadata {
        ...
        let mut metadata = ImportMetadata {
            title: if !parts.is_empty() { parts[0].to_string() } else { filename.to_string() },
            authors: Vec::new(),
            year: None,
            publisher: None,
            source: None,
    ...
    fn extract_from_json_file(&self, file_path: &Path) -> Result<ImportMetadata> {
        ...
        if json_path.exists() {
            ...
            let json_value: Value = serde_json::from_str(&json_content)
                .map_err(|e| TagboxError::Serialization(e))?;
    ...
    fn parse_json_metadata(&self, json: Value) -> Result<ImportMetadata> {
        let mut metadata = ImportMetadata {
            ...
            source: json.get("source")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
            category1: json.get("category1")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string())
                .unwrap_or(self.config.import.metadata.default_category.clone()),
            category2: json.get("category2")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
            category3: json.get("category3")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
            tags: json.get("tags")
                .and_then(|v| v.as_array())
        ...
        if let Some(obj) = json.as_object() {
            for (key, value) in obj.iter() {
                if !["title", "authors", "year", "publisher", "source", 
                     "category1", "category2", "category3", "tags", "summary"]
                    .contains(&key.as_str()) {
                    if let Some(value_str) = value.as_str() {
                        metadata.additional_info.insert(key.clone(), value_str.to_string());
                    }
                }
            }
        }
    ...
    fn merge_metadata(&self, base: ImportMetadata, override_data: ImportMetadata) -> ImportMetadata {
        ImportMetadata {
            title: if override_data.title.is_empty() { base.title } else { override_data.title },
            authors: if override_data.authors.is_empty() { base.authors } else { override_data.authors },
            ...
            category1: if override_data.category1.is_empty() { base.category1 } else { override_data.category1 },
            category2: if override_data.category2.is_none() { base.category2 } else { override_data.category2 },
            category3: if override_data.category3.is_none() { base.category3 } else { override_data.category3 },
            tags: {
