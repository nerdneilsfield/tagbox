...
async fn test_epub_metadata_extraction() {
    ...
    // 验证文件元数据存在
    assert!(metadata.file_metadata.is_some(), "File metadata should exist");
    ...
    assert!(epub_meta.get("spine_count").is_some(), "Should have spine count");
    assert!(epub_meta.get("has_cover").is_some(), "Should have cover info");
    
    // 验证类型元数据
    assert!(metadata.type_metadata.is_some(), "Type metadata should exist");
    ...
    println!("File metadata: {}", serde_json::to_string_pretty(&file_meta).unwrap());
    println!("Type metadata: {}", serde_json::to_string_pretty(&type_meta).unwrap());
}
...
async fn test_pdf_metadata_extraction() {
    ...
    assert!(pdf_meta.get("note").is_some(), "Should have note about pending implementation");
...
mod metadata_format_tests {
    ...
    fn test_type_metadata_structure() {
        ...
        assert!(paper_metadata["paper"]["keywords"].as_array().unwrap().len() == 3);
    }
}